...
export default function GoalsPage() {
  ...
  const fetchGoals = async () => {
    ...
    try {
      ...
      // 可以按截止日期排序，或者按创建时间
      activeGoals.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    ...
    <div className="flex-1 p-4 md:p-8 pt-6 max-w-5xl mx-auto w-full space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl shadow-sm transition-all hover:shadow hover:-translate-y-0.5"
        >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center p-12 text-center rounded-2xl border border-dashed border-gray-200 bg-gray-50/50">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">还没有活跃的长期目标</h3>
          <p className="text-sm text-gray-500 max-w-sm mb-6">
            设定一个大目标（例如读完一本书、完成一次大作业），然后通过拉取片段来循序渐进地完成它。
          </p>
          <button
            onClick={() => setIsEditorOpen(true)}
